...
//            log.info(disasterInfoCodeWithStorageInformation.getJsonObject().toString());
    ...
    public PostVo<JSONObject> informationPredictionStorgeRecord(HttpServletRequest request) {
    ...
    public GetVo<BasicEarthquakeInfo> disasterInfoAll(HttpServletRequest request) {
        ...
        return new GetVo<>(0, "获取基本灾情数据成功！", size, disasterInfos);
    }
    ...
    public GetVo<BasicEarthquakeInfo> getDisaster() {
        int size = basicEarthquakeInfoMapper.getAllDisasterInfo().size();
    ...
    public List<BasicEarthquakeInfo> disasterInfoCopy(@PathVariable("time") int time) {
        return basicEarthquakeInfoMapper.getCopyDisasterInfo(time * 24);
    }

    @GetMapping("/v1/disasterInfo/{time}")
    public GetVo<BasicEarthquakeInfo> disasterInfoByTime(@PathVariable("time") int time, HttpServletRequest request) {
        int limit = Integer.parseInt(request.getParameter("limit"));
        int page = Integer.parseInt(request.getParameter("page"));
...
//    }
    ...
    public PostVo<BasicEarthquakeInfo> editDisasterInfo(HttpServletRequest request, @PathVariable("id") String id) {
        ...
        disasterInfo.setLongitude(Float.parseFloat(request.getParameter("longitude")));
        disasterInfo.setLatitude(Float.parseFloat(request.getParameter("latitude")));
        disasterInfo.setDepth(Float.parseFloat(request.getParameter("depth")));
        disasterInfo.setMagnitude(Float.parseFloat(request.getParameter("magnitude")));
    ...
    public PostVo<BasicEarthquakeInfo> delDisasterInfo(@PathVariable("id") String id) {
        basicEarthquakeInfoMapper.deleteById(id);
        return new PostVo<>(0, "删除成功!", null);
    }

    @GetMapping("/v1/provEarthquakeFrequency")
    public GetVo<ProvEarthquakeFrequency> getProvEarthquakeFrequency() {
        List<ProvEarthquakeFrequency> provEarthquakeFrequencies = basicEarthquakeInfoMapper.getProvEarthquakeFrequency();
        return new GetVo<>(0, "获取数据成功！", provEarthquakeFrequencies.size(), provEarthquakeFrequencies);
    }
